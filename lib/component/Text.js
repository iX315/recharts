...
var Text = function Text(_ref5) {
  ...
  switch (verticalAnchor) {
    case 'start':
      startDy = "calc(".concat(capHeight, ")");
      break;
    case 'middle':
      startDy = "calc(".concat((wordsByLines.length - 1) / 2, " * -").concat(lineHeight, " + (").concat(capHeight, " / 2))");
      break;
    default:
      startDy = "calc(".concat(wordsByLines.length - 1, " * -").concat(lineHeight, ")");
      break;
  }
  var transforms = [];
  if (scaleToFit) {
