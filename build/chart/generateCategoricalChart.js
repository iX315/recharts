...
exports.generateCategoricalChart = exports.getAxisMapByAxes = void 0;
var react_1 = __importStar(require("react"));
var classnames_1 = __importDefault(require("classnames"));
var lodash_1 = __importStar(require("lodash"));
var tiny_invariant_1 = __importDefault(require("tiny-invariant"));
var getTicks_1 = require("../cartesian/getTicks");
...
var BarUtils_1 = require("../util/BarUtils");
...
var getDisplayedData = function (data, _a, item) {
    ...
    var itemsData = (graphicalItems || []).reduce(function (result, child) {
        ...
        if (itemData && itemData.length) {
            return __spreadArrays(result, itemData);
        }
        return result;
    }, []);
...
function isDomainSpecifiedByUser(domain, allowDataOverflow, axisType) {
    if (axisType === 'number' && allowDataOverflow === true && Array.isArray(domain)) {
        ...
        if (!!domainStart && !!domainEnd && DataUtils_1.isNumber(domainStart) && DataUtils_1.isNumber(domainEnd)) {
            return true;
        }
    }
    return false;
...
var getTooltipContent = function (state, chartData, activeIndex, activeLabel) {
    ...
    return graphicalItems.reduce(function (result, child) {
        ...
        var _a = child.props.data, data = _a === void 0 ? chartData : _a;
        var payload;
...
var getAxisMapByAxes = function (props, _a) {
    ...
    var axisMap = axes.reduce(function (result, child) {
        var _a;
        var _b, _c;
        var _d = child.props, type = _d.type, dataKey = _d.dataKey, allowDataOverflow = _d.allowDataOverflow, allowDuplicatedCategory = _d.allowDuplicatedCategory, scale = _d.scale, ticks = _d.ticks, includeHidden = _d.includeHidden;
        ...
        if (isDomainSpecifiedByUser(child.props.domain, allowDataOverflow, type)) {
            domain = ChartUtils_1.parseSpecifiedDomain(child.props.domain, null, allowDataOverflow);
        ...
        if (!domain || domain.length === 0) {
            var childDomain = (_b = child.props.domain) !== null && _b !== void 0 ? _b : defaultDomain;
            if (dataKey) {
                ...
                if (type === 'category' && isCategorical) {
                    ...
                    if (allowDuplicatedCategory && duplicate) {
                        ...
                        domain = ChartUtils_1.parseDomainOfCategoryAxis(childDomain, domain, child).reduce(function (finalDomain, entry) {
                            return finalDomain.indexOf(entry) >= 0 ? finalDomain : __spreadArrays(finalDomain, [entry]);
                        }, []);
                    }
                }
                    ...
                    if (!allowDuplicatedCategory) {
                        domain = ChartUtils_1.parseDomainOfCategoryAxis(childDomain, domain, child).reduce(function (finalDomain, entry) {
                            return finalDomain.indexOf(entry) >= 0 || entry === '' || lodash_1.default.isNil(entry)
                                ? finalDomain
                                : __spreadArrays(finalDomain, [entry]);
                        }, []);
                    }
                    else {
                    ...
                    var errorBarsDomain = ChartUtils_1.parseErrorBarsOfAxis(displayedData, graphicalItems.filter(function (item) { return item.props[axisIdKey] === axisId && (includeHidden || !item.props.hide); }), dataKey, axisType, layout);
                    if (errorBarsDomain) {
                ...
                domain = ChartUtils_1.getDomainOfItemsWithSameAxis(displayedData, graphicalItems.filter(function (item) { return item.props[axisIdKey] === axisId && (includeHidden || !item.props.hide); }), type, layout, true);
            }
            if (type === 'number') {
                domain = DetectReferenceElementsDomain_1.detectReferenceElementsDomain(children, domain, axisId, axisType, ticks);
                if (childDomain) {
                    domain = ChartUtils_1.parseSpecifiedDomain(childDomain, domain, allowDataOverflow);
                }
            }
            else if (type === 'category' && childDomain) {
                var axisDomain_1 = childDomain;
                var isDomainValid = domain.every(function (entry) { return axisDomain_1.indexOf(entry) >= 0; });
                if (isDomainValid) {
                    domain = axisDomain_1;
                }
            }
        ...
        return __assign(__assign({}, result), (_a = {}, _a[axisId] = __assign(__assign({}, child.props), { axisType: axisType,
            ...
            duplicateDomain: duplicateDomain, originalDomain: (_c = child.props.domain) !== null && _c !== void 0 ? _c : defaultDomain, isCategorical: isCategorical,
            layout: layout }), _a));
    }, {});
    return axisMap;
};
    ...
    var axisMap = graphicalItems.reduce(function (result, child) {
        ...
        if (!result[axisId]) {
            ...
            if (isCategorical) {
                ...
                domain = ChartUtils_1.parseSpecifiedDomain(originalDomain, ChartUtils_1.getDomainOfItemsWithSameAxis(displayedData, graphicalItems.filter(function (item) { return item.props[axisIdKey] === axisId && !item.props.hide; }), 'number', layout), Axis.defaultProps.allowDataOverflow);
                domain = DetectReferenceElementsDomain_1.detectReferenceElementsDomain(children, domain, axisId, axisType);
            }
            return __assign(__assign({}, result), (_a = {}, _a[axisId] = __assign(__assign({ axisType: axisType }, Axis.defaultProps), { hide: true, orientation: lodash_1.default.get(ORIENT_MAP, axisType + "." + index % 2, null), domain: domain,
...
var getAxisMap = function (props, _a) {
    ...
    var axisMap = {};
    if (axes && axes.length) {
        axisMap = exports.getAxisMapByAxes(props, {
            axes: axes,
            graphicalItems: graphicalItems,
            axisType: axisType,
            axisIdKey: axisIdKey,
...
var tooltipTicksGenerator = function (axisMap) {
    ...
    return {
        ...
        tooltipAxis: axis,
        tooltipAxisBandSize: ChartUtils_1.getBandSizeOfAxis(axis, tooltipTicks),
    };
};
var createDefaultState = function (props) {
    var _a, _b;
    var children = props.children, defaultShowTooltip = props.defaultShowTooltip;
    var brushItem = ReactUtils_1.findChildByType(children, Brush_1.Brush);
...
var calculateOffset = function (_a, prevLegendBBox) {
    ...
    var brushItem = ReactUtils_1.findChildByType(children, Brush_1.Brush);
    var legendItem = ReactUtils_1.findChildByType(children, Legend_1.Legend);
...
var generateCategoricalChart = function (_a) {
    ...
    var getFormatItems = function (props, currentState) {
        ...
        graphicalItems.forEach(function (item, index) {
            ...
            var axisObj = axisComponents.reduce(function (result, entry) {
                ...
                tiny_invariant_1.default((axisMap && axisMap[id]) || entry.axisType === 'zAxis', "Specifying a(n) " + entry.axisType + "Id requires a corresponding " + entry.axisType + "Id on the targeted graphical component " + ((_c = (_b = item === null || item === void 0 ? void 0 : item.type) === null || _b === void 0 ? void 0 : _b.displayName) !== null && _c !== void 0 ? _c : ''));
                var axis = axisMap[id];
            ...
            if (itemIsBar) {
                ...
                var barBandSize_1 = (_c = (_b = ChartUtils_1.getBandSizeOfAxis(cateAxis, cateTicks, true)) !== null && _b !== void 0 ? _b : maxBarSize) !== null && _c !== void 0 ? _c : 0;
            ...
            if (composedFn) {
                formattedItems.push({
                    props: __assign(__assign({}, composedFn(__assign(__assign({}, axisObj), { displayedData: displayedData,
                        props: props,
                        dataKey: dataKey,
                        item: item,
                        bandSize: bandSize,
    ...
    var updateStateOfAxisMapsOffsetAndStackGroups = function (_a, prevState) {
        ...
        var formattedGraphicalItems = getFormatItems(props, __assign(__assign({}, axisObj), { dataStartIndex: dataStartIndex,
            dataEndIndex: dataEndIndex,
            updateId: updateId,
            graphicalItems: graphicalItems,
            stackGroups: stackGroups,
            offset: offset }));
        return __assign(__assign({ formattedGraphicalItems: formattedGraphicalItems,
            graphicalItems: graphicalItems,
            offset: offset,
    ...
    return _b = (function (_super) {
            ...
            function CategoricalChartWrapper(props) {
                ...
                _this.accessibilityManager = new AccessibilityManager_1.AccessibilityManager();
                ...
                _this.verticalCoordinatesGenerator = function (_a) {
                    ...
                    return ChartUtils_1.getCoordinatesOfGrid(getTicks_1.getTicks(__assign(__assign(__assign({}, CartesianAxis_1.CartesianAxis.defaultProps), xAxis), { ticks: ChartUtils_1.getTicksOfAxis(xAxis, true), viewBox: { x: 0, y: 0, width: width, height: height } })), offset.left, offset.left + offset.width);
                };
                _this.horizontalCoordinatesGenerator = function (_a) {
                    var yAxis = _a.yAxis, width = _a.width, height = _a.height, offset = _a.offset;
                    return ChartUtils_1.getCoordinatesOfGrid(getTicks_1.getTicks(__assign(__assign(__assign({}, CartesianAxis_1.CartesianAxis.defaultProps), yAxis), { ticks: ChartUtils_1.getTicksOfAxis(yAxis, true), viewBox: { x: 0, y: 0, width: width, height: height } })), offset.top, offset.top + offset.height);
                };
                ...
                _this.renderCursor = function (element) {
                    ...
                    var cursorProps = __assign(__assign(__assign(__assign({ stroke: '#ccc', pointerEvents: 'none' }, offset), restProps), ReactUtils_1.filterProps(element.props.cursor)), { payload: activePayload, payloadIndex: activeTooltipIndex, key: key, className: 'recharts-tooltip-cursor' });
                ...
                _this.renderPolarAxis = function (element, displayName, index) {
                    ...
                    var axisOption = axisMap && axisMap[element.props[axisType + "Id"]];
                ...
                _this.renderPolarGrid = function (element) {
                    ...
                    return react_1.cloneElement(element, {
                        polarAngles: lodash_1.isArray(polarAngles)
                            ? polarAngles
                            : ChartUtils_1.getTicksOfAxis(angleAxis, true).map(function (entry) { return entry.coordinate; }),
                        polarRadius: lodash_1.isArray(polarRadius)
                            ? polarRadius
                            : ChartUtils_1.getTicksOfAxis(radiusAxis, true).map(function (entry) { return entry.coordinate; }),
                        cx: cx,
                        cy: cy,
                ...
                _this.renderTooltip = function () {
                    ...
                    var tooltipItem = ReactUtils_1.findChildByType(children, Tooltip_1.Tooltip);
                    if (!tooltipItem) {
                        return null;
                    }
                ...
                _this.renderActivePoints = function (_a) {
                    ...
                    var dotProps = __assign(__assign({ index: childIndex, dataKey: dataKey, cx: activePoint.x, cy: activePoint.y, r: 4, fill: ChartUtils_1.getMainColorOfGraphicItem(item.item), strokeWidth: 2, stroke: '#fff', payload: activePoint.payload, value: activePoint.value, key: key + "-activePoint-" + childIndex }, ReactUtils_1.filterProps(activeDot)), types_1.adaptEventHandlers(activeDot));
                ...
                _this.renderActiveBar = function (_a) {
                    var item = _a.item, childIndex = _a.childIndex;
                    var key = item.props.key;
                    var _b = item.item.props, activeBar = _b.activeBar, dataKey = _b.dataKey;
                    var inheritedProps = __assign(__assign({}, item.item.props), item.props.data[childIndex]);
                    var barProps = __assign(__assign(__assign({ index: childIndex, dataKey: dataKey, fill: ChartUtils_1.getMainColorOfGraphicItem(item.item), strokeWidth: 2, stroke: '#fff', key: key + "-activeBar-" + childIndex }, inheritedProps), ReactUtils_1.filterProps(activeBar)), types_1.adaptEventHandlers(activeBar));
                    return react_1.default.createElement(BarUtils_1.ActiveBar, __assign({ option: activeBar }, barProps));
                };
                _this.renderGraphicChild = function (element, displayName, index) {
                    ...
                    var _c = item.item.props, activeDot = _c.activeDot, hide = _c.hide, activeBar = _c.activeBar;
                    var hasActive = !hide && isTooltipActive && tooltipItem && (activeDot || activeBar) && activeTooltipIndex >= 0;
                    var itemEvents = {};
                    ...
                    if (hasActive) {
                        ...
                        if (activeBar) {
                            return [graphicalItem, _this.renderActiveBar({ item: item, childIndex: activeTooltipIndex })];
                        }
                ...
                _this.renderCustomized = function (element, displayName, index) {
                    return react_1.cloneElement(element, __assign(__assign({ key: "recharts-customized-" + index }, _this.props), _this.state));
                };
            ...
            CategoricalChartWrapper.prototype.componentDidMount = function () {
                ...
                this.accessibilityManager.setDetails({
                    ...
                    offset: {
                        left: (_a = this.props.margin.left) !== null && _a !== void 0 ? _a : 0,
                        top: (_b = this.props.margin.top) !== null && _b !== void 0 ? _b : 0,
                    },
            ...
            CategoricalChartWrapper.prototype.getSnapshotBeforeUpdate = function (prevProps, prevState) {
                var _a, _b;
            ...
            CategoricalChartWrapper.prototype.getTooltipEventType = function () {
                var tooltipItem = ReactUtils_1.findChildByType(this.props.children, Tooltip_1.Tooltip);
            ...
            CategoricalChartWrapper.prototype.getMouseInfo = function (event) {
                ...
                var boundingRectWidth = (_a = element === null || element === void 0 ? void 0 : element.getBoundingClientRect()) === null || _a === void 0 ? void 0 : _a.width;
                ...
                var _b = this.state, xAxisMap = _b.xAxisMap, yAxisMap = _b.yAxisMap;
            ...
            CategoricalChartWrapper.prototype.inRange = function (x, y, scale) {
                if (scale === void 0) { scale = 1; }
                var layout = this.props.layout;
                var _a = [x / scale, y / scale], scaledX = _a[0], scaledY = _a[1];
                if (layout === 'horizontal' || layout === 'vertical') {
                    var offset = this.state.offset;
                    var isInRange = scaledX >= offset.left &&
                        scaledX <= offset.left + offset.width &&
                        scaledY >= offset.top &&
                        scaledY <= offset.top + offset.height;
                    return isInRange ? { x: scaledX, y: scaledY } : null;
                }
                var _b = this.state, angleAxisMap = _b.angleAxisMap, radiusAxisMap = _b.radiusAxisMap;
                if (angleAxisMap && radiusAxisMap) {
                    var angleAxis = DataUtils_1.getAnyElementOfObject(angleAxisMap);
                    return PolarUtils_1.inRangeOfSector({ x: scaledX, y: scaledY }, angleAxis);
                }
                return null;
            };
            ...
            CategoricalChartWrapper.prototype.renderAxis = function (axisOptions, element, displayName, index) {
                ...
                return (react_1.default.createElement(CartesianAxis_1.CartesianAxis, __assign({}, axisOptions, { className: classnames_1.default("recharts-" + axisOptions.axisType + " " + axisOptions.axisType, axisOptions.className), key: element.key || displayName + "-" + index, viewBox: { x: 0, y: 0, width: width, height: height }, ticksGenerator: this.axesTicksGenerator })));
            };
            ...
            CategoricalChartWrapper.prototype.render = function () {
                ...
                var attrs = ReactUtils_1.filterProps(others);
                var map = {
                ...
                if (this.props.accessibilityLayer) {
                    attrs.tabIndex = 0 !== null && 0 !== void 0 ? 0 : this.props.tabIndex;
                    attrs.role = 'img' !== null && 'img' !== void 0 ? 'img' : this.props.role;
                    attrs.onKeyDown = function (e) {
                        _this.accessibilityManager.keyboardEvent(e);
                    };
                    attrs.onFocus = function () {
                        _this.accessibilityManager.focus();
                    };
                }
