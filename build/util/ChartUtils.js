...
var d3Scales = __importStar(require("victory-vendor/d3-scale"));
...
var appendOffsetOfLegend = function (offset, items, props, legendBox) {
    ...
    if (legendProps) {
        ...
        if ((layout === 'vertical' || (layout === 'horizontal' && verticalAlign === 'middle')) && DataUtils_1.isNumber(offset[align])) {
...
var getDomainOfErrorBars = function (data, item, dataKey, layout, axisType) {
    ...
    var errorBars = ReactUtils_1.findAllByType(children, ErrorBar_1.ErrorBar).filter(function (errorBarChild) {
...
var parseErrorBarsOfAxis = function (data, items, dataKey, axisType, layout) {
    var domains = items
        .map(function (item) { return exports.getDomainOfErrorBars(data, item, dataKey, layout, axisType); })
...
var getDomainOfItemsWithSameAxis = function (data, items, type, layout, filterNil) {
    var domains = items.map(function (item) {
        ...
        if (type === 'number' && dataKey) {
            return exports.getDomainOfErrorBars(data, item, dataKey, layout) || getDomainOfDataByKey(data, dataKey, type, filterNil);
        }
...
var getTicksOfAxis = function (axis, isGrid, isAll) {
    ...
    offset = axis.axisType === 'angleAxis' && (range === null || range === void 0 ? void 0 : range.length) >= 2 ? DataUtils_1.mathSign(range[0] - range[1]) * 2 * offset : offset;
    if (isGrid && (axis.ticks || axis.niceTicks)) {
        ...
        return result.filter(function (row) { return !lodash_1.default.isNaN(row.coordinate); });
    }
...
var getTicksOfScale = function (scale, opts) {
    ...
    if (tickCount &&
        ...
        scale.domain([lodash_1.default.min(tickValues), lodash_1.default.max(tickValues)]);
...
var getTooltipItem = function (graphicalItem, payload) {
    ...
    return __assign(__assign({}, ReactUtils_1.filterProps(graphicalItem)), { dataKey: dataKey,
        unit: unit,
