...
var Line = (function (_super) {
        ...
        _this.getStrokeDasharray = function (length, totalLength, lines) {
            ...
            for (var i = 0, sum = 0;; sum += lines[i], ++i) {
                if (sum + lines[i] > remainLength) {
                    remainLines = __spreadArrays(lines.slice(0, i), [remainLength - sum]);
                    break;
                }
            }
            var emptyLines = remainLines.length % 2 === 0 ? [0, restLength] : [restLength];
            return __spreadArrays(Line.repeat(lines, count), remainLines, emptyLines).map(function (line) { return line + "px"; }).join(', ');
        };
    ...
    Line.repeat = function (lines, count) {
        var linesUnit = lines.length % 2 !== 0 ? __spreadArrays(lines, [0]) : lines;
        var result = [];
        for (var i = 0; i < count; ++i) {
            result = __spreadArrays(result, linesUnit);
        }
        return result;
    };
    Line.prototype.renderErrorBar = function (needClip, clipPathId) {
        if (this.props.isAnimationActive && !this.state.isAnimationFinished) {
            return null;
        }
        var _a = this.props, points = _a.points, xAxis = _a.xAxis, yAxis = _a.yAxis, layout = _a.layout, children = _a.children;
        var errorBarItems = ReactUtils_1.findAllByType(children, ErrorBar_1.ErrorBar);
        if (!errorBarItems) {
            return null;
        }
        var dataPointFormatter = function (dataPoint, dataKey) {
            return {
                x: dataPoint.x,
                y: dataPoint.y,
        ...
        return (react_1.default.createElement(Layer_1.Layer, __assign({}, errorBarProps), errorBarItems.map(function (item, i) {
    ...
    Line.prototype.renderDots = function (needClip, clipDot, clipPathId) {
        ...
        var lineProps = ReactUtils_1.filterProps(this.props);
        var customDotProps = ReactUtils_1.filterProps(dot, true);
    ...
    Line.prototype.renderCurveStatically = function (points, needClip, clipPathId, props) {
        ...
        var curveProps = __assign(__assign(__assign(__assign({}, ReactUtils_1.filterProps(others, true)), { fill: 'none', className: 'recharts-line-curve', clipPath: needClip ? "url(#clipPath-" + clipPathId + ")" : null, points: points }), props), { type: type,
            layout: layout,
    ...
    Line.prototype.render = function () {
        var _a;
        var _b = this.props, hide = _b.hide, dot = _b.dot, points = _b.points, className = _b.className, xAxis = _b.xAxis, yAxis = _b.yAxis, top = _b.top, left = _b.left, width = _b.width, height = _b.height, isAnimationActive = _b.isAnimationActive, id = _b.id;
        ...
        var _c = (_a = ReactUtils_1.filterProps(dot)) !== null && _a !== void 0 ? _a : { r: 3, strokeWidth: 2 }, _d = _c.r, r = _d === void 0 ? 3 : _d, _e = _c.strokeWidth, strokeWidth = _e === void 0 ? 2 : _e;
        var _f = (ReactUtils_1.isDotProps(dot) ? dot : {}).clipDot, clipDot = _f === void 0 ? true : _f;
